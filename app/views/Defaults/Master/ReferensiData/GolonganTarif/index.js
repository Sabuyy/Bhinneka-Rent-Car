window.defaultUrl = `${baseUrl}/master/referensi-data/golongan-tarif/`;
var table;
...
$(document).ready(function() {
    ...
    modal.find('form').on('submit', function(ev) {
        ...
        let url = type == 'create' ?
            ...
            .fail(function(jqXHR) {
              if (jqXHR.status === 400 && jqXHR.responseJSON && jqXHR.responseJSON.message === "Kode sudah digunakan") {
                notification("error", jqXHR.responseJSON.message);
                notification('danger', "Kode sudah digunakan");
                modal.modal('hide');
                table.ajax.reload();
                return false;
              } else if (jqXHR.status === 400 && jqXHR.responseJSON && jqXHR.responseJSON.message === "Nama sudah digunakan") {
                notification("error", jqXHR.responseJSON.message);
                notification('danger', "Nama sudah digunakan");
                modal.modal('hide');
                table.ajax.reload();
                return false;
