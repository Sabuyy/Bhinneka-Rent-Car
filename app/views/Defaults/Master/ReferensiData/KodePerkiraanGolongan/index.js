window.defaultUrl = `${baseUrl}/master/referensi-data/kode-perkiraan-golongan/`;
var table;
    ...
    $('#btn-edit').on('click', function() {
        ...
        if (selected) {
            ...
            if(selected.parent == 0){
                $('#checkbox-induk').prop('checked', false)
                $("select[name=id_induk]").attr('disabled', true);     
            }else{
                $('#checkbox-induk').prop('checked', true);
                $("select[name=id_induk]").attr('disabled', false);     
            }
            $("select[name=id_induk]").select2("trigger", "select", { data: { id: selected.id, text : selected.parent} });
            resetErrors();
            modal.modal('show');
        }
    });
...
function select2data(id_select){
