window.defaultUrl = `${baseUrl}/master/referensi-data/kode-perkiraan/`;
var table;
$(document).ready(function() {
    ...
    $("#btn-refresh-data").click(function () {
        ...
        $("select[name=search_id_induk]").val(false).trigger( "change" );
    ...
    $('#btn-add').on('click', function() {
        ...
        modal.find("select[name=kelompok]").val('').trigger('change');
    ...
    $('#btn-edit').on('click', function() {
        ...
        if (selected) {
            ...
            if (selected.parent != 0) {
              ...
              if (selected.id_akun_kelompok != null) {
                $("select[name=kelompok]").select2("trigger", "select", { data: { id: selected.id_akun_kelompok, text : selected.nama_akun_kelompok} });
              } else {
                modal.find("select[name=kelompok]").val('').trigger('change');
              }  
            resetErrors();
...
function kelompokS2(){
    $('.select2Kelompok').select2({
        ...
        width: 'auto',
        ajax: {
            url: "{{ url('panel/referensi/getKodePerkiraanKelompok') }}",
...
function indukPerkiraanS2(){
    $('.select2induk').select2({
        allowClear: true,
        theme: "bootstrap4",
        width: 'auto',
        ajax: {
            url: "{{ url('panel/referensi/getIndukKodePerkiraan') }}",
            data: function (params) {
                return {
                    q: params.term,
                    page: params.page || 1
